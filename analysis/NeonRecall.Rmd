...
epsg_df<-data.frame(site=c("SJER","TEAK"),epsg=c("32611","3857"))

#set epsg
epsg_string<-paste("+init=epsg:",epsg_df[epsg_df$site %in% site,"epsg"],sep="")
...
recalls=list()
  for(x in 1:length(plotIDs)){
    recalls[[x]]<-evaluateNeon(trees=trees,plotID=plotIDs[x],path_to_tiles=path_to_tiles,algorithm = "silva",basemap = basemap,proj4=epsg_string,plot_results=T)
  }
...
f<-list.files(paste("/Users/Ben/Documents/DeepForest/data/",site,"/annotations/",sep=""),pattern=paste(site,"_0",sep=""),full.names = T)
